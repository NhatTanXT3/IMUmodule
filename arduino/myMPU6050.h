...
#define GYRO_NOISE_RANGE 30 //use with gyro range: 1000 deg/s
#define ACC_NOISE_RANGE_ 0.15 // m/s^2
#define rad_to_deg  57.29578
#define deg_to_rad  0.01745329

//#define  GYRO_CONVER_FACTOR 32.768 // change the unit from bit to degree per second(dsp) : range 1000dps
#define  GYRO_CONVER_FACTOR 131.072 // change the unit from bit to degree per second(dsp) : range 250dps

#define ACCLEROMETER_SCALE_FACTOR 1680.4 // change the unit from bit to m/s^2) : range 2g (g=9.75)
...
class myMPU6050
{
  public:
    ...
    void init_module();
    void sensorUpdate();
    void MPU6050DataGyroGetRaw (int16_t *pui16X, int16_t *pui16Y, int16_t *pui16Z);
    void MPU6050DataAccelGetRaw (int16_t *pui16X, int16_t *pui16Y, int16_t *pui16Z);
    uint8_t Calib_Gyro();
    uint8_t Calib_Accelerometer_Amplitude();
    void sensorProcess(float Ts);

    static volatile unsigned char flag_MPU6050_INTpin;
    ...
    float accX_linear_filter;
    float accY_linear_filter;
    float accZ_linear_filter;
